...
export async function GET(req) {
  ...
  const tab        = searchParams.get("tab")    || "sin_tomar"; // sin_tomar | mis_leads | todos
  ...
  try {
    ...
    if (["superadmin","admin"].includes(rol)) {
      // Admin ve todo — solo filtrar por tab si es "sin_tomar"
      if (tab === "sin_tomar") {
...
export async function PATCH(req) {
  ...
  try {
    ...
    const {
      ...
      tomar, // acción especial: vendedor toma el lead
    } = body;
    ...
    // Acción "Tomar lead" — asigna al vendedor y mueve a contactado
    if (tomar) {
      const rol = session.user.rol;
      // Solo admin puede asignar a otros, vendedor solo se asigna a sí mismo
      const asignado = ["superadmin","admin"].includes(rol) && asignado_a
        ? asignado_a
        : session.user.id;
      campos.push("asignado_a = ?", "estado = 'contactado'", "tomado_en = NOW()", "fecha_ultimo_contacto = NOW()");
      ...
      if (estado !== undefined) {
        ...
        // Cerrar lead → crear tenant automáticamente
    ...
    // Si se tomó el lead y venía de WhatsApp, vincular la conversación existente
    if (tomar) {
      await db.query(
        `UPDATE ventas_conversaciones
         SET asignado_a = ?, lead_id = ?
         WHERE lead_id = ? OR (contacto_telefono = (SELECT telefono FROM ventas_leads WHERE id = ?) AND lead_id IS NULL)`,
        [session.user.id, id, id, id]
      );
    }
